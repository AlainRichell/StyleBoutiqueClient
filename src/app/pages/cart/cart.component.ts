          ...
          <ng-container *ngIf="items.length > 0">
            <div class="space-y-4">
              ...
              <div *ngFor="let item of items; let i = index" class="border-b pb-4">
                ...
                <!-- Selector de tallas -->
                <div *ngIf="item.talla.length > 0" class="mt-2">
                  <label for="tallas-{{item.idproducto}}" class="block text-sm font-medium text-gray-700 mb-2">
                    Seleccione una talla:
                  </label>
                  <select
                    [(ngModel)]="item.selectedTalla"
                    id="tallas-{{item.idproducto}}"
                    class="w-auto px-4 py-2 border rounded-md p-2 focus:outline-none {{ item.tallaError ? 'border-red-500' : 'border-gray-300' }}"
                    (change)="item.tallaError = !item.selectedTalla">
                    <option value="" disabled selected>-</option>
                    <option *ngFor="let talla of item.talla" [value]="talla.talla">{{ talla.talla }}</option>
                  </select>
                  <p *ngIf="item.tallaError" class="text-red-500 text-sm mt-1">
...
export class CartComponent {
  ...
  validateTallas(items: CartItem[]): boolean {
    let valid = true;
    items.forEach(item => {
      if (item.talla?.length && !item.selectedTalla) {
        item.tallaError = true; // Marca el error
        valid = false;
      } else {
        item.tallaError = false; // Limpia errores previos si ya seleccionó
      }
    });
    return valid;
  }
  ...
  async sendOrder(items: any[]) {
    ...
    const tallasValidas = this.validateTallas(items);

    if (this.fullNameError || this.addressError || !tallasValidas) {
      return;
    }
